...
export const RecipeNode: React.FC<NodeProps> = ({ id, data, xPos, yPos }) => {
  ...
  return (
    <div ref={nodeRef} className={`glass node-container ${isDone ? 'done' : ''}`} style={{
      width: '240px',
      padding: '12px',
      ...
      {/* Input Handle (Left) */}
      <Handle type="target" position={Position.Left} style={{ background: 'var(--accent-primary)', width: '8px', height: '8px' }} />

      <header style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '8px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          {isDone ? 
            <CheckCircle size={16} style={{ color: 'var(--accent-success)', cursor: 'pointer' }} onClick={() => toggleDone(id)} /> : 
            <Circle size={16} style={{ color: 'var(--text-muted)', cursor: 'pointer' }} onClick={() => toggleDone(id)} />
          }
          <span style={{ fontSize: '0.8rem', fontWeight: 700, color: isDone ? 'var(--accent-success)' : 'var(--accent-primary)' }}>
            {quantity}x
          </span>
        </div>
      ...
      <div style={{ fontSize: '0.95rem', fontWeight: 600, color: '#fff', marginBottom: '4px', lineHeight: 1.2 }}>
        {displayName}
      </div>
      ...
      {recipes.length > 1 && !isDone && !dataAdapter.isLeaf(itemId) && (
        <div style={{ margin: '8px 0 4px 0', position: 'relative' }}>
          ...
          <ChevronDown size={10} style={{ position: 'absolute', right: '8px', top: '50%', transform: 'translateY(-50%)', pointerEvents: 'none', opacity: 0.5 }} />
        </div>
      )}

      {benchName && !isDone && (
        <div style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '0.65rem', color: 'rgba(255,255,255,0.3)', marginTop: '8px' }}>
          <Hammer size={10} />
          {benchName}
        </div>
      )}
      ...
      <Handle type="source" position={Position.Right} style={{ background: 'var(--accent-primary)', width: '8px', height: '8px' }} />
      
      {/* Sink Handle (Right Center for Dynamic Linker) */}
      <Handle type="source" position={Position.Right} id="sink" style={{ background: 'transparent', opacity: 0, right: -4 }} />
    </div>
  );
};
