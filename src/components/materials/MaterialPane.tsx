...
export const MaterialPane: React.FC = () => {
  ...
  const materials = React.useMemo(() => getBOM(), [getBOM, itemsRequested, recipeOverrides, checkedNodes]);

  return (
    <div className="scroll-hide" style={{ padding: '32px 24px' }}>
      <header style={{ marginBottom: '24px' }}>
        <h2 style={{ fontSize: '1.25rem', fontWeight: 600, color: '#fff', display: 'flex', alignItems: 'center', gap: '12px' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
        {materials.length === 0 ? (
          ...
          materials.map(mat => (
            <MaterialItem key={mat.id} id={mat.id} count={mat.count} />
          ))
        )}
      </div>
    </div>
